		...
		class avia_post_grid {
			...
			function __construct( $atts = array() ) {
				$this->atts = shortcode_atts( array(
					...
					'set_breadcrumb'      => true,
					//no shortcode option for this, modifies the breadcrumb nav, must be false on taxonomy overview
					'class'               => "",
					'custom_markup'       => '',
					'fullscreen'          => false,
					'query_orderby'       => 'date',
			...
			public function html() {
				...
				foreach ( $this->entries->posts as $entry ) {
					...
					$link_markup    = apply_filters( 'avf_portfolio_custom_image_container', array(
						"a href='{$link}' title='" . esc_attr( strip_tags( $title ) ) . "' ",
						'a'
					), $entry );
					...
					if ( $columns == "1" && $one_column_template == 'special' ) {
						...
						$output .= "<article class='main_color inner-entry' " . avia_markup_helper( array(
								'context'       => 'entry',
								'echo'          => false,
								'id'            => $the_id,
						...
						$output .= ! empty( $excerpt ) ? "<div class='grid-entry-excerpt entry-content' " . avia_markup_helper( array(
								'context'       => 'entry_content',
								'echo'          => false,
								'id'            => $the_id,
								'custom_markup' => $custom_markup
			...
			protected function build_preview_template( $entry ) {
				...
				$output .= "<article class='inner_slide $nogalleryclass' " . avia_markup_helper( array(
						'context'       => 'entry',
						'echo'          => false,
						'id'            => $id,
						'custom_markup' => $this->atts['custom_markup']
					) ) . ">";
				...
				$output .= "<div class='entry-content-wrapper entry-content' " . avia_markup_helper( array(
						'context'       => 'entry_content',
						'echo'          => false,
						'id'            => $id,
						'custom_markup' => $this->atts['custom_markup']
